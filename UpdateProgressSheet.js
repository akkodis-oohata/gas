...

{
  ...
  const COLOR_SETTING_KEYWORDS = [MIMAKI_KEYWORD, BG_AGARI_KEYWORD, MAKIZUMI_KEYWORD];
  ...
  const SCENE_MAKIZUMI_MANHOUR_COLUMN_INDEX = SCENE_MAKIZUMI_MANHOUR_COLUMN_NUM - 1
  const SCENE_MAKIZUMI_NUMBER_COLUMN_NUM = 14
  const SCENE_MAKIZUMI_NUMBER_COLUMN_INDEX = SCENE_MAKIZUMI_NUMBER_COLUMN_NUM - 1

  const SCENE_REARANGE_MANHOUR_COLUMN_NUM = 15
  const SCENE_REARANGE_MANHOUR_COLUMN_INDEX = SCENE_REARANGE_MANHOUR_COLUMN_NUM - 1
  ...
  const SCENE_INTERRUPT_CHECK_COLUMN_INDEX = SCENE_INTERRUPT_CHECK_COLUMN_NUM - 1;
  const SCENE_LAST_CHANGE_VALUE_COLUMN_NUM = 19; //前回算出増減
  const SCENE_LAST_CHANGE_VALUE_COLUMN_INDEX = SCENE_LAST_CHANGE_VALUE_COLUMN_NUM - 1;
  const SCENE_REPLACE_SELECT_CHECK_COLUMN_NUM = 20  //置換シーン選択チェック列
  const SCENE_REPLACE_SELECT_CHECK_COLUMN_INDEX = SCENE_REPLACE_SELECT_CHECK_COLUMN_NUM - 1;
  ...
  const ERROR_DIALOG_MESSAGE_PREFIX = "エラーが見つかりました。下記Noを修正してください\n";
  ...
  const ERROR_MESSAGE_EXCLUSION_ROW_NOT_FOUND = '「欠番、Bank、全セル」の行が見つかりませんでした。\nシーン列に「欠番、Bank、全セル」を追加してください。';
  const ERROR_MESSAGE_EXCLUSION_COLOR_NOT_SET = "「欠番、Bank、全セル」の色が設定されていません。";
  const ERROR_MESSAGE_WARNING = "警告:"
  const ERROR_MESSAGE_UI_ACCESS_ERROR = "UIにアクセスできません:"
  const ERROR_DIALOG_MESSAGE_MAN_HOUR_LIMIT = "工数は150日以上表示できません\n表示できないシーン"
  const ERROR_MESSAGE_COLOR_MISSING = "塗りつぶし色設定が存在しないので設定してください。: "
  const ERROR_MESSAGE_COLOR_IS_CLEAR =  "塗りつぶし色設定が透明です。透明以外に設定してください。: "
  ...
  class Scene{
    ...
    constructor(name, color, rowNumber, rearangeManDay, person, startDate, loStatus, lastChangeValueColor){
      //シーン名
      this.name = name
      //行番号
  ...
  function getAllDataAndBackground(sheet) {
    ...
    // 背景色を取得する
    allBackgrounds = sheet.getRange(1, 1, dataValues.length, dataValues[0].length).getBackgrounds();
    // シーン列の背景色がついている数を取得する。
    rowsUntilTransparent = getRowsUntilTransparent(allBackgrounds, SCENE_COLUMN_INDEX, SCENE_START_BODY_ROW_INDEX);
  }

  function getRowsUntilTransparent(allBackgrounds, columnIndex, startRowIndex = 0) {
    // 指定された列の背景色を取得する
    let columnBackgrounds = allBackgrounds.slice(startRowIndex).map(row => row[columnIndex]); 

    // 透明な背景色が見つかるまでの行数を取得する
    let rowCount = 0;
    for (let i = 0; i < columnBackgrounds.length; i++) {
      if (columnBackgrounds[i] === COLOR_CLEAR) { // 透明な背景色の場合
        break;
      }
      rowCount++;
    }
  ...
  function clearSheetSections(sheet){
    ...
    clearContentsAndBackgrounds(sheet, SCENE_START_BODY_ROW_INDEX, SCENE_LO_STATUS_COLUMN_NUM, SCENE_MAKIZUMI_NUMBER_COLUMN_NUM)
    //合計列だけ消す
    sheet.getRange(SCENE_ALL_SCENE_TOTAL_ROW_NUM, SCENE_LO_STATUS_COLUMN_NUM, 1, SCENE_MAKIZUMI_NUMBER_COLUMN_NUM)
    .clearContent()
    .setBackground(COLOR_TOTAL_BACKGROUND);
    //未納Cut,合計LO未入り、合計LO入りを初期化
    sheet.getRange(SCENE_TOTAL_LO_ROW_NUM, SCENE_TOTAL_MINOU_NUMBER_COLUMN_NUM)
    .clearContent()
    .setBackground(COLOR_TOTAL_BACKGROUND);
    sheet.getRange(SCENE_TOTAL_LO_ROW_NUM, SCENE_TOTAL_LOMIIRI_NUMBER_COLUMN_NUM, 1, 2)
    .clearContent()
    .setBackground(COLOR_TOTAL_BACKGROUND);
    sheet.getRange(SCENE_TOTAL_LO_ROW_NUM, SCENE_TOTAL_LO_NUMBER_COLUMN_NUM, 1, 2)
    .clearContent()
    .setBackground(COLOR_TOTAL_BACKGROUND);
    //未入・未撒き合計％を初期化
    sheet.getRange(SCENE_TOTAL_MIIRIMIMAKI_ROW_NUM, SCENE_TOTAL_MIIRIMIMAKI_MANDAY_COLUMN_NUM, 1, 3)
    .clearContent()
    .setBackground(COLOR_MIIRIMIMAKI_TOTAL_BACKGROUND);
    //前回算出増減列を初期化
    sheet.getRange(SCENE_START_BODY_ROW_NUM,SCENE_LAST_CHANGE_VALUE_COLUMN_NUM,rowsUntilTransparent,1)
    .clearContent()
    .setBackground(COLOR_CLEAR)
    //グラフの塗りつぶし範囲L列～BI列まで
    clearContentsAndBackgrounds(sheet, SCENE_START_BODY_ROW_NUM, SCENE_START_MANHOUR_COLUMN_NUM, SCENE_END_MANHOUR_COLUMN_NUM)
  ...
  function checkDataConsistency(sheet){
    ...
    while(true){
      ...
      while(true){
        ...
        let dualUse = String(rowArray[cutNoColumnIndex + CUT_DUALUSE_COLUMN_FROM_NO_NUM]);

        //兼用列が"/○○" や "/○○/○○" の形式であるかチェック（空欄でない場合のみ）
        if(dualUse.length > 0 && !/^\/[^\/]+(\/[^\/]+)*$/.test(dualUse)){
          errorMessages.push(`No ${no}: ${ERROR_MESSAGE_INVALID_DUALUSE_FORMAT}`);
        }
        positionIndex++
      }
  ...
  function CheckColorSettings(sheet){
    ...
    if(miiriColor == COLOR_CLEAR) errorMessages.push(ERROR_MESSAGE_COLOR_MIIRI_NOT_SET);
    if(mimakiColor == COLOR_CLEAR) errorMessages.push(ERROR_MESSAGE_COLOR_MIMAKI_NOT_SET);
    if(makizumiColor == COLOR_CLEAR) errorMessages.push(ERROR_MESSAGE_COLOR_MAKIZUMI_NOT_SET);
    ...
    if (exclusionRowIndex === -1) {
      ...
      if(exclusionColor == COLOR_CLEAR) errorMessages.push(ERROR_MESSAGE_EXCLUSION_COLOR_NOT_SET);
    }
  ...
  function drawProgressCells(sheet) {
    ...
    sceneArray.forEach(scene => {
      ...
      if(makiZumiManDay > 0) {
        sheet.getRange(scene.rowNumber,startPosition,1,makiZumiManDay).setBackground(makizumiColor);
      ...
      if(mimakiManDay > 0) {
        sheet.getRange(scene.rowNumber,startPosition,1,mimakiManDay).setBackground(mimakiColor);
      ...
      if(miiriManDay > 0) {
        sheet.getRange(scene.rowNumber,startPosition,1,miiriManDay).setBackground(miiriColor);
      ...
      // 残りの部分を白でクリア
      if (remainingDays > 0) {
        sheet.getRange(scene.rowNumber, startPosition, 1, remainingDays).setBackground(COLOR_CLEAR);
      }
  ...
  function drawTable(sheet){
    ...
    sceneArray.forEach(scene => {
      ...
      range = sheet.getRange(scene.rowNumber,SCENE_MAKIZUMI_MANHOUR_COLUMN_NUM)
      ...
      //前回算出増減を記入する。背景色も入れる。
      range = sheet.getRange(scene.rowNumber,SCENE_LAST_CHANGE_VALUE_COLUMN_NUM)
      range.setValue(scene.lastChangeValue).setBackground(scene.lastChangeValueColor)

      if(scene.name === EXCLUSION_KEYWORD){
        //「欠番、Bank、全セル」のカット数記入
        let range = sheet.getRange(scene.rowNumber,SCENE_CUT_NUMBER_COLUMN_NUM)
    ...
    SceneTotals.totalLOMiiriPercentage = SceneTotals.totalLOMiiriNumber / SceneTotals.drawNumber
    SceneTotals.totalLONumber = SceneTotals.mimakiNumber + SceneTotals.makizumiNumber
    SceneTotals.totalLOPercentage = SceneTotals.totalLONumber / SceneTotals.drawNumber
    //未入・未撒き合計％を算出
    SceneTotals.totalMiiriMimakiNumber = SceneTotals.miiriNumber + SceneTotals.makizumiNumber
    SceneTotals.totalMiiriMimakiManDay = SceneTotals.miiriManDay + SceneTotals.mimakiManDay
    SceneTotals.totalMiiriMimakiPercentage = SceneTotals.totalMiiriMimakiNumber / SceneTotals.drawNumber
  ...
  function setTotalValues(sheet,SceneTotals){
    sheet.getRange(SCENE_ALL_SCENE_TOTAL_ROW_NUM, SCENE_CUT_NUMBER_COLUMN_NUM).setValue(SceneTotals.cut)
    sheet.getRange(SCENE_ALL_SCENE_TOTAL_ROW_NUM, SCENE_BG_COLUMN_NUM).setValue(SceneTotals.deliveryCut)
    sheet.getRange(SCENE_ALL_SCENE_TOTAL_ROW_NUM, SCENE_TOTAL_MANHOUR_COLUMN_NUM).setValue(SceneTotals.totalManDay)
    .setFontWeight('bold')
    sheet.getRange(SCENE_ALL_SCENE_TOTAL_ROW_NUM, SCENE_DRAW_COLUMN_NUM).setValue(SceneTotals.drawNumber)
    .setFontWeight('bold')
    sheet.getRange(SCENE_ALL_SCENE_TOTAL_ROW_NUM, SCENE_MIIRI_MANHOUR_COLUMN_NUM).setValue(SceneTotals.miiriManDay)
    sheet.getRange(SCENE_ALL_SCENE_TOTAL_ROW_NUM, SCENE_MIIRI_NUMBER_COLUMN_NUM).setValue(SceneTotals.miiriNumber)
    sheet.getRange(SCENE_ALL_SCENE_TOTAL_ROW_NUM, SCENE_MIMAKI_MANHOUR_COLUMN_NUM).setValue(SceneTotals.mimakiManDay)
    sheet.getRange(SCENE_ALL_SCENE_TOTAL_ROW_NUM, SCENE_MIMAKI_NUMBER_COLUMN_NUM).setValue(SceneTotals.mimakiNumber)
    sheet.getRange(SCENE_ALL_SCENE_TOTAL_ROW_NUM, SCENE_MAKIZUMI_MANHOUR_COLUMN_NUM).setValue(SceneTotals.makizumiManDay)
    sheet.getRange(SCENE_ALL_SCENE_TOTAL_ROW_NUM, SCENE_MAKIZUMI_NUMBER_COLUMN_NUM).setValue(SceneTotals.makizumiNumber)
    sheet.getRange(SCENE_ALL_SCENE_TOTAL_ROW_NUM, SCENE_REARANGE_MANHOUR_COLUMN_NUM).setValue(SceneTotals.rearangeManDay)
    //未納Cut、合計LO未入り、合計LO入りを算出
    sheet.getRange(SCENE_TOTAL_LO_ROW_NUM, SCENE_TOTAL_MINOU_NUMBER_COLUMN_NUM).setValue(SceneTotals.minouNumber)
    sheet.getRange(SCENE_TOTAL_LO_ROW_NUM, SCENE_TOTAL_LOMIIRI_NUMBER_COLUMN_NUM).setValue(SceneTotals.totalLOMiiriNumber)
    sheet.getRange(SCENE_TOTAL_LO_ROW_NUM, SCENE_TOTAL_LOMIIRI_PERCENTAGE_COLUMN_NUM).setValue(SceneTotals.totalLOMiiriPercentage)
    sheet.getRange(SCENE_TOTAL_LO_ROW_NUM, SCENE_TOTAL_LO_NUMBER_COLUMN_NUM).setValue(SceneTotals.totalLONumber)
    sheet.getRange(SCENE_TOTAL_LO_ROW_NUM, SCENE_TOTAL_LO_PERCENTAGE_COLUMN_NUM).setValue(SceneTotals.totalLOPercentage)
    //未入・未撒き合計％を算出
    sheet.getRange(SCENE_TOTAL_MIIRIMIMAKI_ROW_NUM, SCENE_TOTAL_MIIRIMIMAKI_MANDAY_COLUMN_NUM).setValue(SceneTotals.totalMiiriMimakiManDay)
    sheet.getRange(SCENE_TOTAL_MIIRIMIMAKI_ROW_NUM, SCENE_TOTAL_MIIRIMIMAKI_NUMBER_COLUMN_NUM).setValue(SceneTotals.totalMiiriMimakiNumber)
    sheet.getRange(SCENE_TOTAL_MIIRIMIMAKI_ROW_NUM, SCENE_TOTAL_MIIRIMIMAKI_PERCENTAGE_COLUMN_NUM).setValue(SceneTotals.totalMiiriMimakiPercentage)
  }
  ...
  function setLoStatusAndBackgroundColor(sheet, scene){
    ...
    */
    ...
    if (scene.BGNumber === scene.cutNumber) {
      //T1済
      sheet.getRange(scene.rowNumber, startColumn, 1, endColumn - startColumn + 1)
      .setBackground(COLOR_LO_STATUS_T1)
      sheet.getRange(scene.rowNumber, startColumn)
      .setValue(T1_KEYWORD)
  ...
  function createSceneClasses(sheet) {
    ...
    while(true){
      ...
      let newScene = new Scene(name, background, positionIndex + 1, rearangeManDay, person, startDate)
      sceneArray.push(newScene)
      
      //前回算出増減列用にシーンクラスを作成する
      let beforeScene = new Scene(name, background, positionIndex + 1, rearangeManDay, person, startDate)
  ...
  function updateSceneClassesByBlock(sheet, cutNoColumnIndex){
    ...
    while(true){
      ...
      let color = backgroundArray[cutNoColumnIndex + CUT_SCENE_COLUMN_FROM_NO_NUM]
      let manHour = rowArray[cutNoColumnIndex + CUT_MANHOUR_COLUMN_FROM_NO_NUM]

      let noColor = backgroundArray[cutNoColumnIndex]
      let makizumiColor = backgroundArray[cutNoColumnIndex + CUT_MAKIZUMI_COLUMN_FROM_NO_NUM]
      
      let dualUse = String(rowArray[cutNoColumnIndex + CUT_DUALUSE_COLUMN_FROM_NO_NUM]);
      dualUseArray.push(...dualUse.split('/').filter(Boolean)); 

      let bgAgariColor = backgroundArray[cutNoColumnIndex + CUT_BGAGARI_COLUMN_FROM_NO_NUM]
      
      sceneArray.forEach(scene => {
        if(scene.color == color ){
          caclulateNumber(scene, noColor, exclusionColor, no, dualUseArray, bgAgariColor, makizumiColor, manHour)
        }
      })
  ...
  //欠番、Bank、全セル以外か確認
  function caclulateNumber(scene, noColor, exclusionColor, no, dualUseArray, bgAgariColor, makizumiColor, manHour){
  ...
  function updateSceneClasses(sheet) {
    ...
    sceneArray.forEach(scene => {
      ...
      let beforeScene = beforeSceneArray.find(beforeScene => beforeScene.name === scene.name)
      scene.lastChangeValue =  scene.totalManDay - beforeScene.totalManDay
      scene.lastChangeValueColor = lastChangeValueColorSelect(scene,beforeScene)
    })
    ...
    function lastChangeValueColorSelect(scene, beforeScene) {
      ...
      else if (scene.mimakiManDay !== beforeScene.mimakiManDay || 
  ...
  function convertSceneManHourToDay(scene) {
    ...
    scene.totalManDay = scene.miiriManDay + scene.mimakiManDay + scene.makizumiManDay  //総工数は0.25で切り上げ後を足し合わせたものとする。
  }
  ...
  function getBeforeSceneClasses(){
    ...
    beforeSceneArray.forEach(scene => {
      setNumericValueOrDefault(scene, 'averageCutsPerDay', parseFloat(dataValues[scene.rowNumber - 1][SCENE_AVERAGE_CUT_COLUMN_INDEX]), 0);
      setNumericValueOrDefault(scene, 'cutNumber', parseFloat(dataValues[scene.rowNumber - 1][SCENE_CUT_NUMBER_COLUMN_INDEX]), 0);
      setNumericValueOrDefault(scene, 'BGNumber', parseFloat(dataValues[scene.rowNumber - 1][SCENE_BG_COLUMN_INDEX]), 0);
      setNumericValueOrDefault(scene, 'drawNumber', parseFloat(dataValues[scene.rowNumber - 1][SCENE_DRAW_COLUMN_INDEX]), 0);
      setNumericValueOrDefault(scene, 'totalManDay', parseFloat(dataValues[scene.rowNumber - 1][SCENE_TOTAL_MANHOUR_COLUMN_INDEX]), 0);
      setNumericValueOrDefault(scene, 'miiriManDay', parseFloat(dataValues[scene.rowNumber - 1][SCENE_MIIRI_MANHOUR_COLUMN_INDEX]), 0);
      setNumericValueOrDefault(scene, 'mimakiManDay', parseFloat(dataValues[scene.rowNumber - 1][SCENE_MIMAKI_MANHOUR_COLUMN_INDEX]), 0);
      setNumericValueOrDefault(scene, 'makizumiManDay', parseFloat(dataValues[scene.rowNumber - 1][SCENE_MAKIZUMI_MANHOUR_COLUMN_INDEX]), 0);
      setNumericValueOrDefault(scene, 'miiriNumber', parseFloat(dataValues[scene.rowNumber - 1][SCENE_MIIRI_NUMBER_COLUMN_INDEX]), 0);
      setNumericValueOrDefault(scene, 'mimakiNumber', parseFloat(dataValues[scene.rowNumber - 1][SCENE_MIMAKI_NUMBER_COLUMN_INDEX]), 0);
      setNumericValueOrDefault(scene, 'makizumiNumber', parseFloat(dataValues[scene.rowNumber - 1][SCENE_MAKIZUMI_NUMBER_COLUMN_INDEX]), 0);
      setNumericValueOrDefault(scene, 'rearangeManDay', parseFloat(dataValues[scene.rowNumber - 1][SCENE_REARANGE_MANHOUR_COLUMN_INDEX]), 0);
      scene.rearangeTotalManDay = scene.totalManDay + scene.rearangeManDay
    });
  }
